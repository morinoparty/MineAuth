...
Header values are checked before they are written. A value that contains a carriage return, a line feed, or a NUL character is dropped and logged instead of being sent, so an `ETag` or a custom header built from user input cannot inject additional headers. Framing headers (`Content-Type`, `Content-Length`, `Transfer-Encoding`, `Connection`) are always managed by MineAuth and cannot be overridden from `headers`.
