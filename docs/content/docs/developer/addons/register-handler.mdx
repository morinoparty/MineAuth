...
class ShopHandler {
    ...
    @GetMapping("/shops/{id}")
    suspend fun getShop(@PathParam("id") id: String): ShopData? {
        // Return single shop
...
Use `@QueryParams` to receive all query parameters:
...
class ShopHandler : KoinComponent {
    ...
    @GetMapping("/shops")
    suspend fun listShops(@QueryParams params: Map<String, String>): List<ShopData> {
