...
{
  "paths": {
    "/api/v1/plugins/vaultaddon/balance/me": {
      "get": {
...
Endpoints with `@AuthedAccessUser` annotation are marked with OAuth2 security requirements in the OpenAPI spec. Public endpoints (without authentication) have no security requirements.
